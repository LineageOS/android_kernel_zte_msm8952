...
/*
 *  trace buffer phy addr, must confirm with boot for
		the correct addr for every platform
 */

u64 sdlog_mem_phys_addr = 0;
u32 sdlog_mem_size = 0;

static void *sdlog_mem_phys_imem_addr;
static void *sdlog_mem_size_imem_addr;
...
static int is_f3mem_range(unsigned long pfn, unsigned long size)
{
	...
	if ((from < sdlog_mem_phys_addr) ||
			(to > (sdlog_mem_phys_addr + sdlog_mem_size)))
		return 0;
...
{
	pr_err("msm_tb_read_mem - not supported\n");
	return -EFAULT;
}
...
static int msm_tb_mmap_mem(struct file *file, struct vm_area_struct *vma)
{
	...
	pr_info("msm_tb_mmap_mem-vm_start:0x%lx,vm_end:0x%lx,pgoff:0x%lx\n",
	       vma->vm_start, vma->vm_end, vma->vm_pgoff);

	if (!valid_mmap_phys_addr_range(vma->vm_pgoff, size)) {
		pr_err("valid_mmap_phys_addr_range - failed\n");
		return -EINVAL;
	}

	if (!private_mapping_ok(vma)) {
		pr_err("private_mapping_ok - failed\n");
		return -ENOSYS;
	}

	if (!range_is_allowed(vma->vm_pgoff, size)) {
		pr_err("range_is_allowed - failed\n");
		return -EPERM;
	}

	if (!phys_mem_access_prot_allowed(file, vma->vm_pgoff, size,
					  &vma->vm_page_prot)) {
		pr_err("phys_mem_access_prot_allowed - failed\n");
		return -EINVAL;
	}
	...
	if (remap_pfn_range(vma,
		...
		pr_err("msm_tb_mmap_mem - remap_pfn_range failed\n");
		return -EAGAIN;
	}

	pr_info("msm_tb_mmap_mem - remap_pfn_range succeed\n");
	return 0;
}
...
{
	pr_err("msm_tb_memory_lseek - not supported\n");
	return -EINVAL;
}
...
static int __init msm_tb_dev_init(void)
{
	...
	if (!np) {
		...
		pr_info("unable to map imem sdlog mem size offset\n");
	}
	...
	pr_info("msm_tb_dev_init\n");

	msm_tb_class = class_create(THIS_MODULE, "msm_tb");
	if (IS_ERR(msm_tb_class))
		return PTR_ERR(msm_tb_class);

	device_create(msm_tb_class, NULL,
		MKDEV(MSM_TB_MAJOR, minor), NULL, "msm_tb");


	pr_info("msm_tb_dev_init - succeed\n");
	return 0;
}
